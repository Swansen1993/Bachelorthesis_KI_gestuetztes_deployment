    ...
    async def list_by_filters(
        ...
        for art in all_articles:
            ...
            user_res = await session.execute(
                select(User).where(User.id == art.author_id)
            )
            ...
            fav_count = await session.scalar(
                select(func.count(Favorite.article_id)).where(
                    Favorite.article_id == art.id
                )
            )
            is_fav = bool(
            ...
            filtered.append(self._to_article_feed_record_dto(row))
